...
#elif defined(__GLIBC__) && !defined(__ANDROID_API__) && !defined(__EMSCRIPTEN__)
// glibc
...
std::string_view::iterator find_classic_impl(std::string_view source, CuCb cu_cb,
                                             CpCb cp_cb)
{
    ...
    {
        auto sv =
            detail::make_string_view_from_iterators<char>(it, source.end()).substr(0, 8);
        ...
        {
            auto tmp = detail::make_string_view_from_iterators<char>(it, source.end());
...
std::string_view::iterator find_nondecimal_digit_simple_impl(std::string_view source)
{
...
std::string_view::iterator find_classic_space_narrow_fast(std::string_view source)
{
...
std::string_view::iterator find_classic_nonspace_narrow_fast(std::string_view source)
{
    return find_classic_impl(
        source, [](char ch) { return !is_ascii_space(ch); },
        [](char32_t cp) { return !detail::is_cp_space(cp); });
...
std::string_view::iterator find_nondecimal_digit_narrow_fast(std::string_view source)
{
...
template bool basic_scan_file_buffer<stdio_file_interface>::sync(std::ptrdiff_t);
...
protected:
    ...
    scan_expected<std::ptrdiff_t> parse(T& value, const CharT* src, Strtod strtod_cb)
    {
        CharT* end{};
        errno = 0;
    ...
    template <typename CharT>
    const CharT* get_null_terminated_source(contiguous_range_factory<CharT>& input)
    {
        if (!input.stores_allocated_string())
        {
            // TODO: call float_reader::read_source?
            auto first_space = read_until_classic_space(input.view());
            input.assign(std::basic_string<CharT>{input.view().begin(), first_space});
        }
    ...
    SCN_NODISCARD scan_expected<void> check_error(std::ptrdiff_t chars_read, int c_errno,
                                                  T value) const
    {
        ...
        {
            SCN_UNLIKELY_ATTR
            return detail::unexpected_scan_error(scan_error::value_positive_underflow,
        ...
        {
            SCN_UNLIKELY_ATTR
            return detail::unexpected_scan_error(scan_error::value_positive_overflow,
...
struct has_charconv_for<
    Float, std::void_t<decltype(std::from_chars(
               SCN_DECLVAL(const char*), SCN_DECLVAL(const char*), SCN_DECLVAL(Float&)))>>
    : std::true_type
{
};
...
protected:
    SCN_MAYBE_UNUSED scan_expected<std::chars_format>
    get_flags(std::string_view& input) const
    {
...
public:
    ...
    scan_expected<std::ptrdiff_t> operator()(T& value) const
    {
        ...
        const auto result = std::from_chars(
            input_view.data(), input_view.data() + input_view.size(), value, *flags);
...
template <typename CharT, typename T>
scan_expected<std::ptrdiff_t> fast_float_fallback(impl_init_data<CharT> data, T& value)
{
#if SCN_HAS_FLOAT_CHARCONV && !SCN_DISABLE_FROM_CHARS
...
struct fast_float_impl : fast_float_impl_base
{
    ...
    scan_expected<std::ptrdiff_t> operator()(T& value) const
    {
            ...
            m_kind == float_reader_base::float_kind::hex_with_prefix)
        {
            ...
            return fast_float_fallback<CharT>({m_input, m_kind, m_options}, value);
        }

        const auto flags = get_flags();
        const auto view = get_view();
        const auto result =
            fast_float::from_chars(view.data(), view.data() + view.size(), value, flags);
...
template <typename CharT, typename T>
scan_expected<std::ptrdiff_t> dispatch_impl(impl_init_data<CharT> data,
...
scan_expected<std::ptrdiff_t> float_reader<CharT>::parse_value_impl(T& value)
{
    ...
    {
        return detail::unexpected_scan_error(scan_error::value_negative_overflow,
                                             n.error().msg());
    }
    if (n.error().code() == scan_error::value_positive_underflow &&
        m_sign == sign_type::minus_sign)
    {
        return detail::unexpected_scan_error(scan_error::value_negative_underflow,
                                             n.error().msg());
    }
    return n;
}
...
void loop_parse_if_eight_decimal_digits(const char*& p, const char* const end,
                                        uint64_t& val)
{
...
const char* parse_decimal_integer_fast_impl(const char* begin, const char* const end,
                                            uint64_t& val)
{
...
template <typename T>
constexpr bool check_integer_overflow(uint64_t val, size_t digits_count, int base,
                                      bool is_negative)
{
...
template <typename T>
auto parse_decimal_integer_fast(std::string_view input, T& val, bool is_negative)
    -> scan_expected<const char*>
{
    uint64_t u64val{};
    auto ptr = parse_decimal_integer_fast_impl(input.data(), input.data() + input.size(),
                                               u64val);

    auto digits_count = static_cast<size_t>(ptr - input.data());
    if (SCN_UNLIKELY(check_integer_overflow<T>(u64val, digits_count, 10, is_negative)))
    {
        return detail::unexpected_scan_error(is_negative
...
template <typename CharT, typename T>
auto parse_regular_integer(std::basic_string_view<CharT> input, T& val, int base,
                           bool is_negative) -> scan_expected<const CharT*>
{
    ...
    while (begin != end)
    {
        ...
        u64val = static_cast<uint64_t>(base) * u64val + static_cast<uint64_t>(digit);
        ++begin;
    }

    auto digits_count = static_cast<size_t>(begin - input.data());
    if (SCN_UNLIKELY(check_integer_overflow<T>(u64val, digits_count, base, is_negative)))
    {
        return detail::unexpected_scan_error(is_negative
                                                 ? scan_error::value_negative_overflow
                                                 : scan_error::value_positive_overflow,
...
template <typename CharT, typename T>
auto parse_integer_value(std::basic_string_view<CharT> source, T& value, sign_type sign,
                         int base)
    -> scan_expected<typename std::basic_string_view<CharT>::iterator>
{
    ...
    const auto end = source.data() + source.size();
    {
        ...
        {
            value = 0;
            return ranges::next(source.begin(), ranges::distance(source.data(), start));
        }
    }
    ...
    SCN_TRY(ptr, parse_regular_integer(detail::make_string_view_from_pointers(start, end),
                                       value, base, sign == sign_type::minus_sign));
    return ranges::next(source.begin(), ranges::distance(source.data(), ptr));
}
...
void parse_integer_value_exhaustive_valid(std::string_view source, T& value)
{
    ...
    while (std::distance(p, end) >= 8)
    {
        SCN_EXPECT(is_word_made_of_eight_decimal_digits_fast(get_eight_digits_word(p)));
...

    {
        ...
        SCN_EXPECT(check_integer_overflow<T>(u64val, digits_count, 10, negative_sign) ==
                   false);
    }
...
#define SCN_DEFINE_INTEGER_READER_TEMPLATE(CharT, IntT)                                  \
    template auto parse_integer_value(std::basic_string_view<CharT> source, IntT& value, \
                                      sign_type sign, int base)                          \
        -> scan_expected<typename std::basic_string_view<CharT>::iterator>;

#if !SCN_DISABLE_TYPE_SCHAR
SCN_DEFINE_INTEGER_READER_TEMPLATE(char, signed char)
SCN_DEFINE_INTEGER_READER_TEMPLATE(wchar_t, signed char)
template void parse_integer_value_exhaustive_valid(std::string_view, signed char&);
#endif
...
template void parse_integer_value_exhaustive_valid(std::string_view, long long&);
#endif
...
template void parse_integer_value_exhaustive_valid(std::string_view, unsigned char&);
#endif
...
template void parse_integer_value_exhaustive_valid(std::string_view, unsigned short&);
#endif
...
template void parse_integer_value_exhaustive_valid(std::string_view, unsigned int&);
#endif
...
template void parse_integer_value_exhaustive_valid(std::string_view, unsigned long&);
#endif
...
template void parse_integer_value_exhaustive_valid(std::string_view, unsigned long long&);
#endif
...
template <typename CharT>
constexpr bool
is_simple_single_argument_format_string(std::basic_string_view<CharT> format)
{
    if (format.size() != 2)
    {
        return false;
...
template <typename CharT>
scan_expected<std::ptrdiff_t>
scan_simple_single_argument(std::basic_string_view<CharT> source,
                            ...
                            detail::locale_ref loc = {})
{
    ...
    auto reader = impl::default_arg_reader<impl::basic_contiguous_scan_context<CharT>>{
        ranges::subrange<const CharT*>{source.data(), source.data() + source.size()},
        SCN_MOVE(args), loc};
...
template <typename CharT>
scan_expected<std::ptrdiff_t>
scan_simple_single_argument(detail::basic_scan_buffer<CharT>& source,
                            basic_scan_args<detail::default_context<CharT>> args,
                            basic_scan_arg<detail::default_context<CharT>> arg,
                            detail::locale_ref loc = {})
{
...
public:
    ...
    constexpr specs_handler(detail::format_specs& specs,
                            basic_scan_parse_context<char_type>& parse_ctx, Context& ctx)
...
struct format_handler_base
{
    ...
    {
        {
            ...
            {
                return on_error(
                    {scan_error::invalid_format_string, "Argument list not exhausted"});
            }
        }
        ...
        for (auto it = visited_args_upper.begin(); it != visited_args_upper.end() - 1;
             ++it)
        {
    ...
    {
        SCN_UNLIKELY_ATTR
        error = detail::unexpected_scan_error(scan_error::invalid_format_string, msg);
    }
    ...
    SCN_NODISCARD bool has_arg_been_visited(size_t id)
    {
        ...
        {
            on_error({scan_error::invalid_format_string, "Argument ID out-of-range"});
            return false;
        }
...
using context_wrapper_t =
    std::conditional_t<Contiguous, contiguous_context_wrapper<CharT>,
...
struct format_handler : format_handler_base
{
    ...
    format_handler(Source&& source, format_type format, args_type args,
                   detail::locale_ref loc, std::size_t argcount)
        : format_handler_base{argcount}, parse_ctx{format},
    ...
    void on_literal_text(const char_type* begin, const char_type* end)
    {
        for (; begin != end; ++begin)
        {
            ...
            else if (is_space)
            {
                ...
                get_ctx().advance_to(impl::read_while_classic_space(get_ctx().range()));
                // And, skip all whitespace in the format string
                auto begin_it =
                    impl::read_while_classic_space(detail::make_string_view_from_pointers(
                        detail::to_address(after_space_it), detail::to_address(end)));
    ...
    void on_replacement_field(std::size_t arg_id, const char_type*)
    {
        ...
        on_visit_scan_arg(impl::default_arg_reader<context_type>{get_ctx().range(),
                                                                 get_ctx().args(),
                                                                 get_ctx().locale()},
                          arg);
    }

    const char_type* on_format_specs(std::size_t arg_id, const char_type* begin,
                                     const char_type* end)
    {
        auto arg = get_arg(get_ctx(), arg_id, *this);
        set_arg_as_visited(arg_id);
        ...
        if (detail::get_arg_type(arg) == detail::arg_type::custom_type)
        {
            ...
            on_visit_scan_arg(
                impl::custom_reader<detail::default_context<char_type>>{parse_ctx,
                                                                        get_custom_ctx()},
                arg);
        ...
        {
            SCN_UNLIKELY_ATTR
            on_error({scan_error::invalid_format_string, "Missing '}' in format string"});
            return parse_ctx.begin();
        }
        ...
        on_visit_scan_arg(
            impl::arg_reader<context_type>{get_ctx().range(), specs, get_ctx().locale()},
            arg);
        return parse_ctx.begin();
    }
...
scan_expected<std::ptrdiff_t>
vscan_parse_format_string(std::basic_string_view<CharT> format, Handler& handler)
{
...
template <typename CharT>
scan_expected<std::ptrdiff_t>
vscan_internal(std::basic_string_view<CharT> source, std::basic_string_view<CharT> format,
               basic_scan_args<detail::default_context<CharT>> args,
               detail::locale_ref loc = {})
{
...
scan_expected<std::ptrdiff_t> vscan_internal(
    detail::basic_scan_buffer<CharT>& buffer, std::basic_string_view<CharT> format,
    basic_scan_args<detail::default_context<CharT>> args, detail::locale_ref loc = {})
{
    const auto argcount = args.size();
    if (is_simple_single_argument_format_string(format) && argcount == 1)
    {
    ...
    {
        auto handler = format_handler<true, CharT>{
            buffer.get_contiguous(), format, SCN_MOVE(args), SCN_MOVE(loc), argcount};
        return vscan_parse_format_string(format, handler);
    }

    SCN_UNLIKELY_ATTR
    {
        auto handler = format_handler<false, CharT>{buffer, format, SCN_MOVE(args),
...
scan_expected<std::ptrdiff_t>
vscan_value_internal(Source&& source, basic_scan_arg<detail::default_context<CharT>> arg)
{
    ...
    -> scan_expected<std::string_view::iterator>
{
    ...
    return reader.read_default_with_base(ranges::subrange{beg, source.end()}, value,
                                         base);
}
...
scan_expected<void> vinput(std::string_view format, scan_args args)
{
    ...
    if (n)
    {
        ...
        {
            return detail::unexpected_scan_error(scan_error::invalid_source_state,
...
template <typename Source>
scan_expected<std::ptrdiff_t> sync_after_vscan(Source& source,
...
scan_expected<std::ptrdiff_t> vscan_impl(std::string_view source, std::string_view format,
                                         scan_args args)
{
    return vscan_internal(source, format, args);
}
scan_expected<std::ptrdiff_t> vscan_impl(scan_buffer& source, std::string_view format,
                                         scan_args args)
{
...
scan_expected<std::ptrdiff_t> vscan_impl(std::wstring_view source,
                                         std::wstring_view format, wscan_args args)
{
    return vscan_internal(source, format, args);
}
scan_expected<std::ptrdiff_t> vscan_impl(wscan_buffer& source, std::wstring_view format,
                                         wscan_args args)
{
    auto n = vscan_internal(source, format, args);
    return sync_after_vscan(source, n);
...
template <typename Locale>
scan_expected<std::ptrdiff_t> vscan_localized_impl(const Locale& loc, scan_buffer& source,
...
template <typename Locale>
scan_expected<std::ptrdiff_t>
vscan_localized_impl(const Locale& loc, std::wstring_view source,
                     std::wstring_view format, wscan_args args)
{
    return vscan_internal(source, format, args, detail::locale_ref{loc});
}
template <typename Locale>
scan_expected<std::ptrdiff_t>
vscan_localized_impl(const Locale& loc, wscan_buffer& source, std::wstring_view format,
                     wscan_args args)
{
    auto n = vscan_internal(source, format, args, detail::locale_ref{loc});
    return sync_after_vscan(source, n);
...
template auto vscan_localized_impl<std::locale>(const std::locale&, std::string_view,
                                                std::string_view, scan_args)
    -> scan_expected<std::ptrdiff_t>;
template auto vscan_localized_impl<std::locale>(const std::locale&, scan_buffer&,
                                                std::string_view, scan_args)
    -> scan_expected<std::ptrdiff_t>;
template auto vscan_localized_impl<std::locale>(const std::locale&, std::wstring_view,
                                                std::wstring_view, wscan_args)
    -> scan_expected<std::ptrdiff_t>;
template auto vscan_localized_impl<std::locale>(const std::locale&, wscan_buffer&,
                                                std::wstring_view, wscan_args)
    -> scan_expected<std::ptrdiff_t>;
#endif
...
scan_expected<std::ptrdiff_t> vscan_value_impl(std::wstring_view source,
                                               basic_scan_arg<wscan_context> arg)
{
    return vscan_value_internal(source, arg);
}
scan_expected<std::ptrdiff_t> vscan_value_impl(wscan_buffer& source,
                                               basic_scan_arg<wscan_context> arg)
{
    auto n = vscan_value_internal(source, arg);
    return sync_after_vscan(source, n);
...
template auto scan_int_impl(std::string_view, unsigned short&, int)
    -> scan_expected<std::string_view::iterator>;
template auto scan_int_exhaustive_valid_impl(std::string_view) -> unsigned short;
#endif
#if !SCN_DISABLE_TYPE_UINT
...
template auto scan_int_impl(std::string_view, unsigned long long&, int)
    -> scan_expected<std::string_view::iterator>;
template auto scan_int_exhaustive_valid_impl(std::string_view) -> unsigned long long;
#endif
...
struct datetime_setter<std::tm>
{
    ...
    static void set_sec(Handler& h, std::tm& t, setter_state& st, int s)
    {
        ...
        {
            return h.set_error(
                {scan_error::invalid_scanned_value, "Invalid value for tm_sec"});
        }
        t.tm_sec = s;
        st.set_sec(h);
    }
    ...
    static void set_min(Handler& h, std::tm& t, setter_state& st, int m)
    {
        ...
        {
            return h.set_error(
                {scan_error::invalid_scanned_value, "Invalid value for tm_min"});
        }
        t.tm_min = m;
        st.set_min(h);
    }
    ...
    static void set_hour24(Handler& hdl, std::tm& t, setter_state& st, int h)
    {
        if (SCN_UNLIKELY(h < 0 || h > 23))
        {
            return hdl.set_error(
                {scan_error::invalid_scanned_value, "Invalid value for tm_hour"});
        }
        t.tm_hour = h;
        st.set_hour24(hdl);
    }
    ...
    static void set_hour12(Handler& hdl, std::tm& t, setter_state& st, int h)
    {
        ...
        {
            return hdl.set_error(
                {scan_error::invalid_scanned_value, "Invalid value for 12-hour tm_hour"});
        }
        t.tm_hour = h;
        st.set_hour12(hdl);
    }
    ...
    static void set_mday(Handler& h, std::tm& t, setter_state& st, int d)
    {
        ...
        {
            return h.set_error(
                {scan_error::invalid_scanned_value, "Invalid value for tm_mday"});
        }
        t.tm_mday = d;
        st.set_mday(h);
    }
    ...
    static void set_mon(Handler& h, std::tm& t, setter_state& st, int m)
    {
        ...
        {
            return h.set_error(
                {scan_error::invalid_scanned_value, "Invalid value for tm_mon"});
        }
        t.tm_mon = m - 1;
        st.set_mon(h);
    }
    ...
    static void set_full_year(Handler& h, std::tm& t, setter_state& st, int y)
    {
        ...
        {
            return h.set_error(
                {scan_error::invalid_scanned_value, "Invalid value for tm_year"});
        }
        t.tm_year = y - 1900;
        st.set_full_year(h);
    }
    ...
    static void set_wday(Handler& h, std::tm& t, setter_state& st, int d)
    {
        ...
        {
            return h.set_error(
                {scan_error::invalid_scanned_value, "Invalid value for tm_wday"});
        }
        t.tm_wday = d;
        st.set_wday(h);
    }
    ...
    static void set_yday(Handler& h, std::tm& t, setter_state& st, int d)
    {
        ...
        {
            return h.set_error(
                {scan_error::invalid_scanned_value, "Invalid value for tm_yday"});
        }
        t.tm_yday = d;
        st.set_yday(h);
    }
    ...
    template <typename Handler>
    static void set_tz_offset(Handler& h, std::tm& t, setter_state& st,
                              std::chrono::minutes o)
    {
        if constexpr (mp_valid<has_tm_gmtoff_predicate, std::tm>::value)
        {
            assign_gmtoff(t, std::chrono::duration_cast<std::chrono::seconds>(o));
        }
        else
        {
            return h.set_error(
    ...
    template <typename Handler>
    static void set_tz_name(Handler& h, std::tm&, setter_state&, const std::string&)
    {
        return h.set_error({scan_error::invalid_format_string, "tm_zone not supported"});
    }
...
{
    template <typename Handler>
    static void set_subsec(Handler& h, datetime_components& t, setter_state& st, double s)
    {
        assert(s >= 0.0 && s < 1.0);
        t.subsec = s;
        st.set_subsec(h);
    ...
    template <typename Handler>
    static void set_sec(Handler& h, datetime_components& t, setter_state& st, int s)
    {
        if (SCN_UNLIKELY(s < 0 || s > 60))
        {
            return h.set_error(
                {scan_error::invalid_scanned_value, "Invalid value for seconds"});
        }
        t.sec = s;
        st.set_sec(h);
    }
    template <typename Handler>
    static void set_min(Handler& h, datetime_components& t, setter_state& st, int m)
    {
        if (SCN_UNLIKELY(m < 0 || m > 59))
        {
            return h.set_error(
                {scan_error::invalid_scanned_value, "Invalid value for minutes"});
        }
        t.min = m;
        st.set_min(h);
    }
    template <typename Handler>
    static void set_hour24(Handler& hdl, datetime_components& t, setter_state& st, int h)
    {
        if (SCN_UNLIKELY(h < 0 || h > 23))
        {
            return hdl.set_error(
    ...
    template <typename Handler>
    static void set_hour12(Handler& hdl, datetime_components& t, setter_state& st, int h)
    {
        if (SCN_UNLIKELY(h < 1 || h > 12))
        {
    ...
    template <typename Handler>
    static void set_mday(Handler& h, datetime_components& t, setter_state& st, int d)
    {
        if (SCN_UNLIKELY(d < 1 || d > 31))
        {
            return h.set_error(
    ...
    template <typename Handler>
    static void set_mon(Handler& h, datetime_components& t, setter_state& st, int m)
    {
        if (SCN_UNLIKELY(m < 1 || m > 12))
        {
            return h.set_error(
    ...
    template <typename Handler>
    static void set_full_year(Handler& h, datetime_components& t, setter_state& st, int y)
    {
        t.year = y;
        st.set_full_year(h);
    }
    template <typename Handler>
    static void set_century(Handler& h, datetime_components& t, setter_state& st, int c)
    {
        if (!t.year)
        {
            t.year = c * 100;
    ...
    template <typename Handler>
    static void set_short_year(Handler& h, datetime_components& t, setter_state& st,
                               int y)
    {
        if (!t.year)
        {
    ...
    template <typename Handler>
    static void set_wday(Handler& h, datetime_components& t, setter_state& st, int d)
    {
        if (SCN_UNLIKELY(d < 0 || d > 6))
        {
            return h.set_error(
    ...
    template <typename Handler>
    static void set_yday(Handler& h, datetime_components& t, setter_state& st, int d)
    {
        if (SCN_UNLIKELY(d < 0 || d > 6))
        {
            return h.set_error(
    ...
    template <typename Handler>
    static void set_tz_offset(Handler& h, datetime_components& t, setter_state& st,
                              std::chrono::minutes o)
    {
        t.tz_offset = o;
    ...
    template <typename Handler>
    static void set_tz_name(Handler& h, datetime_components& t, setter_state& st,
                            std::string n)
    {
    ...
    static void handle_short_year_and_century(datetime_components& t, setter_state& st)
    {
        assert(t.year);
...
{
    template <typename Handler>
    static void set_tz_offset(Handler& h, tm_with_tz& t, setter_state& st,
                              std::chrono::minutes o)
    {
        if constexpr (mp_valid<has_tm_gmtoff_predicate, std::tm>::value)
        {
    ...
    template <typename Handler>
    static void set_tz_name(Handler& h, tm_with_tz& t, setter_state& st, std::string n)
    {
        t.tz_name = std::move(n);
        return st.set_tzname(h);
    }
...
{
    template <typename Handler>
    static void set_full_year(Handler& h, year_month& t, setter_state& st, int y)
    {
    ...
    template <typename Handler>
    static void set_short_year(Handler& h, year_month& t, setter_state& st, int y)
    {
...
template <>
struct datetime_setter<year_month_day> : unreachable_datetime_setter<year_month_day>
{
    template <typename Handler>
    static void set_full_year(Handler& h, year_month_day& t, setter_state& st, int y)
    {
        t = year_month_day{year{static_cast<int>(y)}, t.month(), t.day()};
        st.set_full_year(h);
    }
    template <typename Handler>
    static void set_century(Handler& h, year_month_day& t, setter_state& st, int c)
    {
        t = year_month_day{year{static_cast<int>(t.year()) + c * 100}, t.month(),
                           t.day()};
        st.set_century(h);
    }
    template <typename Handler>
    static void set_short_year(Handler& h, year_month_day& t, setter_state& st, int y)
    {
        t = year_month_day{year{static_cast<int>(t.year()) + y}, t.month(), t.day()};
        st.set_short_year(h);
    }
    template <typename Handler>
...
public:
    ...
    tm_reader(Range r, T& t, locale_ref loc)
        : m_range(SCN_MOVE(r)), m_begin(ranges::begin(m_range)), m_tm(t), m_loc(loc)
    {
    }
    ...
    void on_short_year(numeric_system sys = numeric_system::standard)
    {
        ...
        if (m_st.localized && sys != numeric_system::standard)
        {
            if (auto t = read_localized("%Ey", L"%Ey"))
            {
                setter::set_short_year(*this, m_tm, m_st, (t->tm_year + 1900) % 100);
            }
            return;
        }
#endif
    ...
    void on_century(numeric_system sys = numeric_system::standard)
    {
        ...
        if (m_st.localized && sys != numeric_system::standard)
        {
            ...
            {
                setter::set_century(*this, m_tm, m_st, (t->tm_year + 1900) / 100);
            }
            return;
        }
#endif
    ...
    void on_weekday_name()
    {
        ...
        return set_error({scan_error::invalid_scanned_value, "Invalid weekday name"});
    }
    ...
    void on_subsecond(numeric_system sys = numeric_system::standard)
    {
        ...
        auto str_res = scan<std::string>(ranges::subrange{m_begin, m_range.end()},
                                         []() -> decltype(auto)
                                         {
                                             if constexpr (std::is_same_v<CharT, char>)
                                             {
    ...
    void on_tz_name()
    {
        auto res = scan<std::string>(ranges::subrange{m_begin, m_range.end()},
                                     []() -> decltype(auto)
                                     {
                                         if constexpr (std::is_same_v<CharT, char>)
                                         {
    ...
    void on_am_pm()
    {
        ...
        set_error({scan_error::invalid_scanned_value, "Invalid am/pm specifier"});
    }
...
private:
    ...
    int read_classic_unsigned_integer(int min_digits, int max_digits)
    {
        ...
        {
            set_error(
                scan_error{scan_error::invalid_scanned_value, "Too few integer digits"});
            return -1;
        }
        return accumulator;
    }
    ...
    std::optional<OptT>
    try_one_of_str_nocase(std::array<std::pair<std::string_view, OptT>, N>& options)
    {
        auto start_it = m_begin;
        std::size_t options_available = N;
        std::size_t chars_consumed = 0;
        while (options_available >= 1 && options.front().first.size() > chars_consumed)
        {
            std::size_t i = 0;
            if (m_begin == m_range.end())
            {
            ...
            {
                const auto cmp =
                    static_cast<unsigned>(ch ^ options[i].first[chars_consumed]);
                if (options[i].first.size() <= chars_consumed || (cmp != 0 && cmp != 32))
                {
    ...
    localized_read_state& get_localized_read_state()
    {
        if (!m_loc_state)
        {
            ...
            if (!std::has_facet<typename localized_read_state::time_facet_type>(loc))
            {
                loc = std::locale(loc,
                                  new typename localized_read_state::time_facet_type{});
            }
            if (!std::has_facet<typename localized_read_state::numpunct_facet_type>(loc))
            {
                loc = std::locale(loc, new
                                  typename localized_read_state::numpunct_facet_type{});
            }

            m_loc_state = localized_read_state{
                SCN_MOVE(loc),
                &std::use_facet<typename localized_read_state::time_facet_type>(loc),
                &std::use_facet<typename localized_read_state::numpunct_facet_type>(loc),
    ...
    std::optional<std::tm> do_read_localized(std::basic_string_view<CharT> fmt)
    {
        ...
        auto iter = facet.get(begin, end, m_loc_state->dummy_stream, err, &tm, fmt.data(),
                              fmt.data() + fmt.size());
        if ((err & std::ios_base::failbit) != 0)
        {
            set_error(
                {scan_error::invalid_scanned_value, "Failed to scan localized datetime"});
            return std::nullopt;
        }
    ...
    std::optional<std::tm> read_localized(std::string_view fmt, std::wstring_view wfmt)
    {
        if constexpr (std::is_same_v<CharT, char>)
        {
    ...
    {
        set_error({scan_error::invalid_format_string,
...
auto chrono_scan_inner_impl(std::basic_string_view<CharT> fmt, T& t, Context& ctx)
    -> scan_expected<typename Context::iterator>
{
    {
        SCN_TRY(it, detail::internal_skip_classic_whitespace(ctx.range(), false));
        ctx.advance_to(SCN_MOVE(it));
    }

    auto r = detail::tm_reader<T, typename Context::range_type, CharT>(ctx.range(), t,
                                                                       ctx.locale());
...
template auto chrono_scan_impl(std::string_view, tm_with_tz&, scan_context&)
    -> scan_expected<scan_context::iterator>;
template auto chrono_scan_impl(std::string_view, datetime_components&, scan_context&)
    -> scan_expected<scan_context::iterator>;
...
template auto chrono_scan_impl(std::wstring_view, tm_with_tz&, wscan_context&)
    -> scan_expected<wscan_context::iterator>;
template auto chrono_scan_impl(std::wstring_view, datetime_components&, wscan_context&)
    -> scan_expected<wscan_context::iterator>;
